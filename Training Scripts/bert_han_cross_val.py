...
if args.feature_dim==0:
    model = BERT_HAN(args.hidden_size, 768, args.num_labels, args.feature_dim).to(device)
else:
    model = BERT_HAN_feature(args.hidden_size, 768, args.num_labels, args.feature_dim).to(device)
